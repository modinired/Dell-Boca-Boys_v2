...
class CollectiveIntelligenceFramework:
    ...
    async def _update_network_metrics(self):
        ...
        # Skip metric computation if networkx is unavailable or the graph is not initialised
        if self.agent_network is None:
            return
        try:
            # Only compute metrics when there are at least two nodes to avoid divide‑by‑zero
            if len(self.agent_network.nodes) > 1:
                ...
                # For clustering coefficient, convert to undirected graph
                self.collaboration_metrics['clustering_coefficient'] = nx.clustering(self.agent_network.to_undirected())
        except Exception as exc:
            # Log and continue; metrics will be absent on error
            self.logger.warning(f"Failed to compute network metrics: {exc}")
    ...
    async def get_collective_intelligence_status(self) -> Dict[str, Any]:
        ...
        # Compute network density only if a graph is available
        if self.agent_network is not None and self.agent_network.number_of_nodes() > 0:
            density = self.agent_network.number_of_edges() / max(1, self.agent_network.number_of_nodes())
        else:
            density = 0.0

        # Compute average trust across all agent nodes
        if self.agent_nodes:
            trust_values = [np.mean(list(node.trust_scores.values()))
                            for node in self.agent_nodes.values()
                            if node.trust_scores]
            average_trust = np.mean(trust_values) if trust_values else 0.0
        else:
            average_trust = 0.0
